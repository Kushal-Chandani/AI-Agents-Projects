...
# Define global session service to ensure singleton behavior
...
def initialize_adk():
    ...
    print("--- ADK Init: Starting initialization... ---")
    
    # Initialize session service only if not already created
    if _session_service is None:
        _session_service = InMemorySessionService()
        print("--- ADK Init: Created new InMemorySessionService instance. ---")
    else:
        print("--- ADK Init: Reusing existing InMemorySessionService instance. ---")
    ...
    adk_session_key = 'adk_session_id_final_mem_v2'
    ...
    if adk_session_key not in st.session_state:
        ...
        print(f"--- ADK Init: Reusing existing ADK session ID: {session_id} ---")
    
    # Verify or create session
    try:
        session = _session_service.get_session(app_name=APP_NAME_FOR_ADK, user_id=USER_ID, session_id=session_id)
        if not session:
            print(f"--- ADK Init: Session {session_id} not found. Creating new session. ---")
            ...
            print(f"--- ADK Init: Successfully created session {session_id}. ---")
        else:
            print(f"--- ADK Init: Session {session_id} found in InMemorySessionService. ---")
    except Exception as e:
        print(f"--- ADK Init: ERROR - Failed to verify/create session {session_id}: {e} ---")
        logging.exception("ADK Session Service error:")
        raise
    
    print(f"--- ADK Init: Initialization complete. Active Session ID: {session_id} ---")
...
def run_adk_sync(runner: Runner, session_id: str, user_message_text: str) -> str:
    ...
    """
    print(f"--- ADK Run: Attempting to run with session {session_id} ---")
    try:
        return asyncio.run(run_adk_async(runner, session_id, user_message_text))
    except Exception as e:
        if "Session not found" in str(e):
            print(f"--- ADK Run: Session {session_id} not found. Forcing reinitialization with new session ID. ---")
            ...
            if adk_session_key in st.session_state:
                ...
                print(f"--- ADK Run: Cleared old session ID from st.session_state. ---")
            try:
                # Reinitialize with a new session ID
                new_runner, new_session_id = initialize_adk()
                print(f"--- ADK Run: Reinitialized with new session {new_session_id}. Retrying... ---")
                return asyncio.run(run_adk_async(new_runner, new_session_id, user_message_text))
            except Exception as reinit_e:
                error_msg = f"Failed to reinitialize ADK: {reinit_e}"
                print(f"--- ADK Run: ERROR - {error_msg} ---")
                logging.exception("ADK reinitialization failed:")
                return f"Sorry, I couldn't recover the session. Please refresh the app and try again. (Error: {reinit_e})"
        else:
...
st.sidebar.caption(f"**Session ID:** `{st.session_state.get('adk_session_id_final_mem_v2', 'N/A')}`")
